              ...
              <div className="min-w-0 flex flex-col pt-0.5">
                ...
                <div className="flex flex-col gap-0.5 mt-1">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-[7px] font-black uppercase px-1.5 py-0.5 rounded truncate max-w-[120px] transition-all ${isMerchantDiff ? 'ring-1 ring-indigo-500/30 bg-indigo-500/10' : 'bg-black/40'} ${isDuplicate ? 'ring-1 ring-rose-500/50 bg-rose-500/10' : ''} ${isAvoidFlagged ? 'text-rose-500' : 'text-slate-500'}`}>{item.merchant || 'General'}</span>
...
const Ledger: React.FC<LedgerProps> = ({ 
  ...
  useEffect(() => { setBatchSuggestions({}); setIsShowingAISuggestionsOnly(false); }, [viewDate]);
  ...
  return (
    <div className={`pb-32 pt-0 animate-slide-up relative min-h-full flex flex-col ${isCompact ? 'gap-1' : 'gap-1.5'}`}>
      ...
      <div className={`flex items-center justify-between glass p-1 rounded-xl mb-1 mx-0.5 border-white/5 shadow-sm ${isCompact ? 'h-[36px]' : 'h-[40px]'} shrink-0 relative z-[200]`}>
        <div className="flex items-center gap-1 h-full px-1"><button onClick={() => (triggerHaptic(), onMonthChange(-1))} className="p-1 text-slate-500 active:scale-90"><ChevronLeft size={16} strokeWidth={3} /></button><h2 className="text-[9px] font-black text-slate-400 uppercase tracking-widest min-w-[55px] text-center">{monthLabel}</h2><button onClick={() => (triggerHaptic(), onMonthChange(1))} className="p-1 text-slate-500 active:scale-90"><ChevronRight size={16} strokeWidth={3} /></button></div>
        <div className="flex items-center gap-0.5 h-full pr-1">{isShowingAISuggestionsOnly && (<button onClick={() => { triggerHaptic(); setIsShowingAISuggestionsOnly(false); }} className="p-1.5 rounded-lg text-rose-500 active:scale-90 transition-transform bg-rose-500/10 mr-1" title="Clear Neural Filter"><X size={16} strokeWidth={3} /></button>)}<button onClick={() => { triggerHaptic(); onAddIncome(); }} className="p-1.5 rounded-lg text-emerald-500 active:scale-90 transition-transform" title="Add Income"><Banknote size={18} strokeWidth={3} /></button><button onClick={() => { triggerHaptic(); onAddRecord(); }} className="p-1.5 rounded-lg text-brand-accentUi active:scale-90 transition-transform" title="Add Expense"><Plus size={18} strokeWidth={3} /></button><button onClick={() => { triggerHaptic(); setSearchOpen(!isSearchOpen); }} className={`p-1.5 rounded-lg transition-all ${isSearchOpen ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}><Search size={16} /></button></div>
      </div>
      <div className="px-0.5 flex-1 flex flex-col">
        {viewMode === 'list' ? (
          <div className={`bg-brand-surface border border-brand-border ${isCompact ? 'rounded-[20px]' : 'rounded-[28px]'} overflow-hidden shadow-sm flex-1 min-h-[400px]`}>
            ...
            <div className="divide-y divide-brand-border">{filteredRecords.map((rec) => (<SwipeableItem key={rec.id} item={rec} recordType={rec.recordType} currencySymbol={currencySymbol} onDelete={rec.recordType === 'income' ? onDeleteIncome : onDeleteExpense} onEdit={onEditRecord} onUpdateExpense={onUpdateExpense} aiSuggestion={batchSuggestions[rec.id]} density={settings.density || 'Compact'} pendingBills={pendingBills} />))}{filteredRecords.length === 0 && (<div className="py-20 text-center flex flex-col items-center justify-center opacity-30">{isShowingAISuggestionsOnly ? (<><Check size={32} className="text-emerald-500 mb-4" /><p className="text-[10px] font-black uppercase tracking-widest">Protocol Optimization Complete</p><button onClick={() => setIsShowingAISuggestionsOnly(false)} className="mt-4 text-[8px] font-black text-brand-primary uppercase tracking-widest underline">Return to Full Feed</button></>) : (<><FilterX size={32} strokeWidth={1.5} /><p className="text-[10px] font-black uppercase tracking-widest mt-4">Registry Null</p></>)}</div>)}</div>
          </div>
        ) : (
